...
test("runs filesystem, shell, and git tools against a bounded workspace", async () => {
...
test("applies a validated multi-operation patch and leaves files unchanged on validation failure", async () => {
...
test("views image metadata and routes documents through the injected analyzer", async () => {
  ...
  expect(result.document).toMatchObject({ text: "summary", artifact: { kind: "pdf", mimeType: "application/pdf" } })
