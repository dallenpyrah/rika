...
const minimumRowEnd = (total: number, limit: number): number => Math.min(limit, Math.max(0, total))

const resolveRowEnd = (window: RowWindowState, total: number, limit: number): number =>
  window.end === 0 ? total : Math.min(total, Math.max(minimumRowEnd(total, limit), window.end))

const rowWindowStart = (end: number, limit: number): number => Math.max(0, end - limit)

const shiftRowEnd = (window: RowWindowState, delta: number, total: number, limit: number): number => {
  const current = resolveRowEnd(window, total, limit)
  return Math.min(total, Math.max(minimumRowEnd(total, limit), current + delta))
}

const relocateRowEnd = (window: RowWindowState, anchorIndex: number, total: number, limit: number): number => {
  const located =
...
const includeRowEnd = (end: number, index: number, total: number, limit: number): number => {
...
export const internal = { minimumRowEnd, resolveRowEnd, rowWindowStart, shiftRowEnd, relocateRowEnd, includeRowEnd }
