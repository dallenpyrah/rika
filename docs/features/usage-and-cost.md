...
Rika derives model cost from Relay `model.usage.reported` events and exposes totals per Turn, Thread, and terminal session. Only monetary cost reported by the provider is authoritative. Token counts, context windows, and budgets are not prices and do not create cost. Child Run usage is attributed to its parent Turn and Thread by traversing Relay's durable child tree.
