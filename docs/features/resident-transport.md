...
Frames, fragments, feeds, and outbound queues are bounded. Feed overflow or an oversized live event requests an ordered resync from durable state instead of failing the Execution. A reconnect may restore reads, but a client does not automatically resend a mutation when its outcome is unknown. A reconnect that reaches an incompatible resident yields with a restart-required failure instead of replacing it; the restarted runtime reopens the selected Thread from durable state.
