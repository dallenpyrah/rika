...
The fan-out runtime materializes each persisted child override as a deterministic child-specific immutable Relay agent definition before starting that child. The definition applies Relay 0.2.12's supported instruction, model selection and request variant, compaction policy, tool, permission, output-schema, and metadata fields. Fan-out members never replace or mutate the shared root definition, so concurrent main and Oracle children retain their own routes. Every child execution owns a deterministic isolated durable Session, allowing inherited, preset, and explicit compaction policies to survive replay without sharing parent context.
