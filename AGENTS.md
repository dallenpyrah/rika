...
- Rika directly owns four read-only, branch-tracking research submodules: `repos/effect` is `https://github.com/Effect-TS/effect-smol.git` on `main`, `repos/baton` is `https://github.com/In-Time-Tec/batonfx.git` on `main`, `repos/relay` is `https://github.com/In-Time-Tec/relayfx.git` on `main`, and `repos/opentui` is `https://github.com/anomalyco/opentui.git` on `main`. Never import from or edit, format, lint, build, or test these vendored trees. Do not initialize nested Relay or Baton vendors.
