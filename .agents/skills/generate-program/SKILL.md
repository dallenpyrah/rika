...
Before following any `FOLDKIT_ROOT/...` path, resolve `FOLDKIT_ROOT` from the Rika repository root:

1. Prefer `../foldkit`.
2. If Rika later vendors FoldKit, use `repos/foldkit`.
3. If neither exists, say the exemplar repo is unavailable and fall back to installed package types under `apps/web/node_modules/foldkit`.
...
10. **Host embedding**: the program runs inside another app ("a widget in our React app", "embed this in an existing page", "the host needs to control it") → `Runtime.makeElement` plus the `Runtime.embed` lifecycle handle, with Flags for initial data and Ports for ongoing communication in both directions. `FOLDKIT_ROOT/examples/embedding/` is the canonical reference: a plain TypeScript host driving a Foldkit widget end to end
...
- `FOLDKIT_ROOT/packages/typing-game/client/src/`: production multi-page app: Submodels, OutMessage, update/view decomposition, curried handler extraction, subscription patterns, domain modules.
- `FOLDKIT_ROOT/packages/website/src/`: production Foldkit website: page organization, shared view primitives, route-driven rendering, idiomatic domain separation.
...
Read `FOLDKIT_ROOT/examples/counter/src/main.ts`

**Tier 2: Timers, subscriptions, simple stateful apps:**
Read `FOLDKIT_ROOT/examples/stopwatch/src/main.ts` (timer via subscription, `Duration` field pattern) and `FOLDKIT_ROOT/examples/todo/src/main.ts` (CRUD with localStorage via flags)

**Tier 3: Async operations, loading/error states, API calls, form validation:**
Read `FOLDKIT_ROOT/examples/weather/src/main.ts` (HTTP with `HttpClient`) and `FOLDKIT_ROOT/examples/form/src/main.ts` (uses `foldkit/fieldValidation`; see the Form Validation section in Phase 4)

**Tier 4: URL routing, multiple pages, query parameters:**
Read `FOLDKIT_ROOT/examples/routing/src/main.ts` and `FOLDKIT_ROOT/examples/query-sync/src/main.ts`

**Tier 5: Complex state, nested domain models, CRUD, drag-and-drop:**
Read `FOLDKIT_ROOT/examples/shopping-cart/src/main.ts` (nested domain schemas, cart state) and `FOLDKIT_ROOT/examples/kanban/src/main.ts` (CRUD with `Ui.DragAndDrop`, flags restoring from localStorage, subscriptions)

**Tier 6: Submodels, OutMessage, multi-step forms, auth flows, multi-module apps:**
Read `FOLDKIT_ROOT/examples/auth/src/main.ts` (login/signup with Submodels, OutMessage, protected routes) and `FOLDKIT_ROOT/examples/job-application/src/main.ts` (multi-step form with deeply nested Submodels in `step/`, `Ui.DatePicker`, `Ui.FileDrop`, `Ui.Menu`, `Calendar` module for date handling)

**Tier 7: Real-time, WebSocket, Managed Resources, production-grade:**
Read `FOLDKIT_ROOT/packages/typing-game/client/src/update.ts`, then explore its `page/home/` and `page/room/` directories for the full Submodel/OutMessage pattern.
...
- `FOLDKIT_ROOT/examples/ui-showcase/src/main.ts`: root wiring, `Got*` delegation, `toParentMessage` helpers
- `FOLDKIT_ROOT/examples/ui-showcase/src/message.ts`: how UI component Messages are structured
- `FOLDKIT_ROOT/examples/ui-showcase/src/model.ts`: how UI component Models are composed
- `FOLDKIT_ROOT/examples/ui-showcase/src/update.ts`: how UI component updates are delegated
- `FOLDKIT_ROOT/examples/ui-showcase/src/toast.ts`: read when using `Ui.Toast`: Toast is unique in that it's parameterized on a payload schema via `Ui.Toast.make(PayloadSchema)`, returning a typed module you import from
...
- For apps with multiple domain entities referenced across modules, extract shared schemas into `src/domain/` (e.g., `domain/product.ts`, `domain/session.ts`). See the shopping-cart and auth examples for this pattern, and read `FOLDKIT_ROOT/packages/website/src/page/projectOrganization.ts` for guidance on when and how to structure domain modules
...
const Message = S.Union([ClickedSubmit, UpdatedEmail, SucceededLogin, FailedLogin, CompletedFocusInput])
...
Canonical reference: `FOLDKIT_ROOT/examples/form/src/main.ts` (async email uniqueness check with version-based cancellation) and `FOLDKIT_ROOT/examples/job-application/src/step/` (validated multi-step forms across submodels).
...
- Canonical reference: `FOLDKIT_ROOT/examples/job-application/src/step/attachments.ts`
...
- `FOLDKIT_ROOT/examples/weather/src/story.test.ts`: simple Command resolution (happy path + error path)
- `FOLDKIT_ROOT/examples/auth/src/page/loggedOut/page/login.story.test.ts`: Submodel with OutMessage assertions, field validation
- `FOLDKIT_ROOT/packages/website/src/search/story.test.ts`: multi-step interactions (arrow key cycling, stale result handling)
...
- `FOLDKIT_ROOT/examples/weather/src/scene.test.ts`: basic Scene flow with form interaction and Command resolution
- `FOLDKIT_ROOT/examples/auth/src/scene.test.ts`: a multi-page app's root-level Scene driving the login flow through the root view
- `FOLDKIT_ROOT/examples/kanban/src/scene.test.ts`: scoped queries with `within`, `toHaveValue`, explicit test data
...
2. FOLDKIT_ROOT/skills/generate-program/architecture.md
3. FOLDKIT_ROOT/skills/generate-program/conventions.md
4. FOLDKIT_ROOT/skills/generate-program/checklist.md
