...
> **Note:** This file is a snapshot of the architecture as practiced in the live Foldkit codebase. In Rika, resolve live-code references by using `../foldkit` first and `repos/foldkit` if vendored. If anything here contradicts what `FOLDKIT_ROOT/examples/` or `FOLDKIT_ROOT/packages/foldkit/src/` actually do, the live code is canonical. The duplication is intentional so the skill works in downstream projects that haven't vendored the foldkit subtree.
...
- **With args, fallible** (HTTP fetch): `FOLDKIT_ROOT/examples/weather/src/main.ts` (`FetchWeather`)
- **With args, infallible** (random + return): `FOLDKIT_ROOT/examples/kanban/src/command.ts` (`GenerateCardId`)
- **With args, storage**: `FOLDKIT_ROOT/examples/kanban/src/command.ts` (`SaveBoard`)
- **Argless, DOM side effect**: `FOLDKIT_ROOT/examples/kanban/src/command.ts` (`FocusAddCardInput`)
...
const init: Runtime.ApplicationInit<Model, Message, Flags> = (flags) => [{ createdAt: flags.createdAt }, []]
```
...
type UpdateReturn = [Model, ReadonlyArray<Command<Message>>, Option.Option<OutMessage>]

// Child signals to parent
CreatedRoom: ({ roomId, player }) => [model, [], Option.some(SucceededCreateRoom({ roomId, player }))]

// Parent handles OutMessage
GotChildMessage: ({ message }) => {
  const [nextChildModel, childCommands, maybeOutMessage] = Child.update(model.child, message)

  const mappedCommands = childCommands.map(Command.mapEffect(Effect.map((message) => GotChildMessage({ message }))))
...
- **Always-active keyboard input**: `FOLDKIT_ROOT/examples/snake/src/main.ts`
- **Animation-frame-driven game tick**: `FOLDKIT_ROOT/examples/canvas-art/src/main.ts`
- **Conditional WebSocket connection** (active only when a session exists): `FOLDKIT_ROOT/examples/websocket-chat/src/main.ts`
- **Production multi-stream pattern**: `FOLDKIT_ROOT/packages/typing-game/client/src/subscription.ts`
...
Use these instead of raw `document.querySelector`, `setTimeout`, `Date.now()`, or `Math.random()`. They compose naturally inside `Command.define`. For canonical wiring, see `FOLDKIT_ROOT/examples/kanban/src/command.ts` (`FocusAddCardInput` wraps `Dom.focus`) and `FOLDKIT_ROOT/examples/stopwatch/src/main.ts` (`Clock.currentTimeMillis` inside an `Effect.gen`).
...
For the canonical update-handler shapes (the exact `UrlRequest` tag names, how to dispatch `pushUrl` vs an external load Command, and how to derive the route from a `Url`), see `FOLDKIT_ROOT/examples/routing/src/main.ts`.
