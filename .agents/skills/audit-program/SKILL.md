...
Before following any `FOLDKIT_ROOT/...` path, resolve `FOLDKIT_ROOT` from the Rika repository root: prefer `../foldkit`, then `repos/foldkit`, then fall back to installed package types under `apps/web/node_modules/foldkit` and say exemplar source is unavailable.
...
4. **Tier-matching exemplar**. Pick at least one to read alongside the audited code:
   - Tier 1-2 → `FOLDKIT_ROOT/examples/counter/src/main.ts`, `FOLDKIT_ROOT/examples/stopwatch/src/main.ts`
   - Tier 3 → `FOLDKIT_ROOT/examples/weather/src/main.ts`, `FOLDKIT_ROOT/examples/form/src/main.ts`
   - Tier 4 → `FOLDKIT_ROOT/examples/routing/src/main.ts`
   - Tier 5 → `FOLDKIT_ROOT/examples/kanban/src/`
   - Tier 6 → `FOLDKIT_ROOT/examples/auth/src/`, `FOLDKIT_ROOT/examples/job-application/src/`
   - Tier 7 → `FOLDKIT_ROOT/packages/typing-game/client/src/page/room/`
5. **Foldkit UI integration**. If any `Ui.*` is imported, also read `FOLDKIT_ROOT/examples/ui-showcase/src/main.ts` for the wiring pattern.
